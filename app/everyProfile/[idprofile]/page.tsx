...
import { DeleteSkills } from "@/app/profilepage/deleteskills";
import { UpdateProfile } from "@/app/profilepage/updateProfile";
import { UpdateSkills } from "@/app/profilepage/updateskills";
import { Profile } from "@/app/profileUI/profile";
...
export default function Page({ params }: any) {
  ...
  const { isPending, error, data } = useQuery({
    queryKey: ["singlepostData", params.idprofile],
    queryFn: () =>
      fetch(`/api/profile/all/${params.idprofile}`).then((res) => res.json()),
  });

  const { data: data2 } = useQuery({
    queryKey: ["singleskillsData"],
    queryFn: () =>
      fetch(`/api/skills/${params.idprofile}`).then((res) => res.json()),
  });
        ...
        {data.userId === session?.user?.id && (
          <div className="text-center mt-5">
            ...
            <Link href="/skillsPage">
              <Button className="h-14 bg-blue-500 rounded-3xl  ">
                {" "}
                Add your skills
              </Button>{" "}
            </Link>{" "}
